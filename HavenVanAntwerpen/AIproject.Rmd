...
# AI Project 2020-2021

## Hoping to predict a ship's destination and its trajectory time delta.
...
The data isn't always that accurate since a ship's owner will sometimes cancel its ship's beacon, meaning it won't send out its location data.

This project is divided into two parts.

First the data will be processed. This will be done through two steps.The first step is to split the data based on a time delta. 

You have to ask yourself these question, "why is the time difference between two registries sometimes a big difference ?" and  "When would an owner stop the beacon from sending location data ?" The answer is that the owner will stop the transmitting of the beacon when he is asleep or docked. This ofcourse makes the data inaccurate. That's why I first split the dataset on a timedelta. This will make sure that the total trajectory of a ship will be split into real trajectories.

The next split will be done based on how fast the ship is moving. If it is moving below a certain threshold (this can't be zero because the current can't be dismissed), the ship most likely has stopped and reached it's destination, meaning we have to split on this point.


The second part is training a neural network to predict a ship's end destination and time traveled between start and end destination.
To train this neural network, I need to be sure the data is in the correct format.
...
Disclaimer; Please compile the first part of this process through the "AIproject_Notebook.ipynb" file, in a visual studio code environment. I put the code here for readability, but the python code will not compile correctly in an R studio environment.
...
The data has now been processed. From here on the code will compile in R studio.
...
The goal is to hopefully accurately predict a correct destination's latitude and longitude, based on the start latitude and longitude. This
...
#### Predicting the ship's destination
...
A neural network classifier is a software system that predicts the value of a categorical value. 
...
This network is a supervised machine learning algorithm.
KNN works by finding the distances between a query and all the examples in the data, selecting the specified number examples (K) closest to the query, then it votes for the most frequent label or the averages of the labels.

An analogy for KNN is; "show me your friends, and I'll tell you who you are".
...
#### Predicting the time delta

The next goal is to make neural networks to accurately predict the time delta between a ship's starting positioning and a ship's destination positioning.

The time delta in this context is the time traveled between the two places.


First we build the nnet neural network.
...
The goal of this project was hoping to be able to predict if it was possible to determine a ship's destination and the time traveled in a ship's route.

Through nnet and KNN it was possible to accurately predict a ship's destination positioning. This was done through training the model on past trajectories.

The time delta has also been predicted, but this was not as accurate as the ship's destination positioning.
