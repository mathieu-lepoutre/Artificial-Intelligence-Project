...
The data isn't always that accurate since a ship's owner will sometimes cancel its ship's beacon, meaning it won't send out location data anymore.
...
The second part is training a neural network to predict a ship's end destination and time travelled between start and end destination.
...
KNN works by finding the distances between a query and all the examples in the data, selecting the specified number examples (K) closest to the query, then votes for the most frequent label or averages the labels.

An analogy for KNN i; "show me your friends, and I'll tell you who you are".
...
The time delta prediction was not as accurace. I believe this is because there's a lot more variables in play when predicting the time delta.
