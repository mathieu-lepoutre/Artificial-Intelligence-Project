...
The goal is to accurately check if the model can predict a correct destination latitude and longitude, based on the start latitude and longitude. This
will be achieved by training the model on the thousands of trajects that happened in the past.


This prediction can be usefull in many applications and situations.
...
data <- read.csv("processeddata.csv", sep = "")
...
First we need to split the dataset into training and test sets The model will be trained on the trained set, and the accuracy will be tested on the test set.
Then the training control will be defined.
