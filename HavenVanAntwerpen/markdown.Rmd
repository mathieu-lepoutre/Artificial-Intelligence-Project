...
In this paper I will try to taccle this challenge.

For input there is a huge unprocessed dataset containing a ship's ID and it's total trajectory. 
Because a ship's owner will sometimes cancel a ship's beacon, the data isn't always that accurate.

First I try process the data, this is a challenge made up of two steps. I made a paint picture to explain it visually.

The first step is to split the data based on a time delta. 

You have to ask yourself these question, "why is the time difference between two registries sometimes a big difference ?"

"When would an owner stop the beacon from sending location data ?"
If the owner is sleeping, or when it's docked. This makes the data inaccurate. That's why I first split the dataset on a timedelta. 
The total imaginary trajectory of a ship will be split in real trajectories.
When the ship hasn't transmitted in a couple of hours, you can be sure of it that he pulled the plug out of the beacon.

After this split you have a ship's trajectory that also included when the stops. This is why the next split will be on speed delta.
If a ship's speed is under a certain threshold, the ship has reached a destination.

To train my neural network I need to be sure the data is in the correct format.
The preferable format is the ship's id, and each row representing it's start latitude and longitude, and it's stop latitude and longitude. The data should also contain
the time delta between the two positions.
